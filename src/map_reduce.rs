...
{
    Arc::new(MapReduceCache::new(row_fn, &|_params, x| Ok(x), worker))
}
...
where
    ...
    WorkerFn:
        for<'a, 'b> Fn(SearchToken<'a, 'b>, &Params, &JoinKeysT::Ret<'a>, &Row) -> Summary + Sync,
{
    let query: Arc<dyn Query<Tablet, Params, JoinKeysT, Summary>> = make_map_reduce(row_fn, worker);
...
where
    ...
    WorkerFn: for<'a, 'b> Fn(
        ...
        + Sync,
{
    MapReduceCache::new(row_fn, transform_fn, worker).query(tablets, params, join_keys)
}
